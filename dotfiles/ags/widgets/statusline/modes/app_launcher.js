...
function reset() {
  ...
  allApps.value = [...apps.map(AppItem), AppItem({ name: "none" }, -1)];

  // Finds indexes of common apps to be displayed initially
  const commonApps = [];
  allApps.value.map((app, i) => {
...
function update(text) {
  const arr = [];
  allApps.value.slice(0, -1).forEach((app, i) => {
...
const AppItem = (app, i) => {
  const main = Widget.Revealer({
    ...
    attribute: app,
    reveal_child: revArray.bind().as((ri) => ri.includes(i)),
    child: Widget.Label({
      class_name: selectedIndex
        .bind()
        .as((si) => (i === si ? "app selected" : "app")),
